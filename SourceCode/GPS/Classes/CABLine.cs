...
namespace AgOpenGPS
{
    public class CABLine
    {
        ...
        public void DrawABLines()
        {
            ...
            GL.Begin(PrimitiveType.Lines);
            GL.Color4(0.62, 0.47, 0.09, 0.9); // change from 0.0, 0.0, 0.0, 0.7 to fendt braun 0.62, 0.47, 0.09, 0.9 // this should be some kind of outline to the Drive A-B Line
            ...
            GL.LineWidth(lineWidth*9); // change set linewidth from 2-8 to always 8 ////////////////////Still testing
            GL.Begin(PrimitiveType.Lines);
            GL.Color3(0.62f, 0.47f, 0.09f); // change from color magenta(0.95f, 0.20f, 0.950f) to fendt braun(0.62f, 0.47f, 0.09f) // this is the Drive A-B Line color
            GL.Vertex3(currentLinePtA.easting, currentLinePtA.northing, 0.0);
            GL.Vertex3(currentLinePtB.easting, currentLinePtB.northing, 0.0);
            GL.End();
            ...
            if (mf.isSideGuideLines && mf.camera.camSetDistance > mf.tool.width * -200)
            {
                ...
                if (toolOffset == 0)
                {
                    ...
                    GL.Color4(0.62f, 0.47f, 0.09f, 1.0f); //change from GL.Color4(0.9907f, 0.990f, 0.9750f, 0.5f);  to  fendt braun 0.62f, 0.47f, 0.09f // still testing this should be the extra guidline color
                    GL.LineWidth(lineWidth); // change from 2 to lineWidth to be able to make how you like it // this is the extra guidline size
                    GL.Begin(PrimitiveType.Lines);
