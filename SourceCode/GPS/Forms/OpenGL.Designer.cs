        ...
        private void oglMain_Paint(object sender, PaintEventArgs e)
        {
            if (sentenceCounter < 299)
            {
                if (isGPSPositionInitialized)
                {
                        ...
                        if (patches > 0)
                        {
                            ...
                            foreach (var triList in triStrip[j].patchList)
                            {
                                ...
                                if (isDraw)
                                {
                                    ...
                                    if (isDay) GL.Color4((byte)triList[0].easting, (byte)triList[0].northing, (byte)triList[0].heading, (byte)255); // change from (byte)152)removes the transpency
                                    else GL.Color4((byte)triList[0].easting, (byte)triList[0].northing, (byte)triList[0].heading, (byte)(255));// change from (byte)(152 * 0.5)) removes the transpency
                    ...
                    if (patchCounter > 0)
                    {
                        if (isDay) GL.Color4(sectionColorDay.R, sectionColorDay.G, sectionColorDay.B, (byte)255); // change now not fully removes the transpency
                        else GL.Color4(sectionColorDay.R, sectionColorDay.G, sectionColorDay.B, (byte)(255));// change now not fully removes the transpency
                        ...
                        for (int j = 0; j < triStrip.Count; j++)
                        {
                            if (triStrip[j].isDrawing)
                            {
                                ...
                                {
                                    if (isDay) GL.Color4(tool.secColors[j].R, tool.secColors[j].G, tool.secColors[j].B, (byte)255);// change now not fully removes the transpency
                                    else GL.Color4(tool.secColors[j].R, tool.secColors[j].G, tool.secColors[j].B, (byte)(255));// change now not fully removes the transpency
                                }
